#!/usr/bin/env python3
"""
Fine-tuning script for Qwen-VL on car crash video summarization task.
"""
...
sys.path.insert(0, str(project_root / "qwen_vlm"))

from qwen_vlm.src.utils import get_config
from qwen_vlm.src.models import QwenVLWrapper
from qwen_vlm.src.training import LossTracker
...
class VideoSummarizationDataset(Dataset):
    """PyTorch Dataset for video-text summarization pairs for Qwen-VL."""
    ...
    def __init__(
        ...
        Args:
            ...
            processor: Qwen-VL processor for image/text processing
        """
    ...
    def __getitem__(self, idx):
        ...
        # Load frames and use middle frame
        ...
        # Qwen-VL uses chat template format
        messages = [
            {
                "role": "user",
                "content": [
        ...
        # Qwen2VLProcessor handles images directly
        ...
        result = {
            "pixel_values": inputs["pixel_values"].squeeze(0) if len(inputs["pixel_values"].shape) > 1 else inputs["pixel_values"],
            "input_ids": inputs["input_ids"].squeeze(0),
            "attention_mask": inputs["attention_mask"].squeeze(0),
            "video_id": video_id,
            "text_summary": text_summary,
            "user_prompt": prompt  # Store for label masking
        }
...
def collate_fn(batch):
    ...
    pad_token_id = batch[0]["input_ids"][0].item() if len(batch[0]["input_ids"]) > 0 else 0
...
def prepare_labels(input_ids, user_prompts, processor, model, device):
    ...
    # For Qwen, find where assistant response starts
    # The chat template includes both user and assistant messages
    # We need to mask the user part and keep the assistant part
    for i, user_prompt in enumerate(user_prompts):
        try:
            # Find assistant token or position
            # Qwen chat template typically has special tokens marking roles
            input_seq = input_ids[i].cpu().tolist()
            
            # Try to find where assistant response starts
            # This is approximate - Qwen's format may vary
            # Look for common patterns or use a heuristic
            seq_len = (input_ids[i] != pad_token_id).sum().item()
            if seq_len > 0:
                # Heuristic: assume first 60% is prompt, last 40% is response
...
def train_epoch(
    ...
    # Skip gradient scaler for quantized models
...
# ------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Fine-tune Qwen-VL for video summarization")
    ...
    print("=" * 60)
    print("FINE-TUNING QWEN-VL FOR VIDEO SUMMARIZATION")
    print("=" * 60)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        max_grad_norm = config.get("training.max_grad_norm") or config.config["training"].get("max_grad_norm", 0.3)
